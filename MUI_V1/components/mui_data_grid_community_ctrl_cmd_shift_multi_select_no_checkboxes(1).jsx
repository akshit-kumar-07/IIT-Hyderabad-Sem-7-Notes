 ...
 *
 * Visuals:
 * - DataGrid Community may only visually mark the last selected row by default.
 *   To highlight ALL selected rows, we add our own class via `getRowClassName`
 *   and style it with `sx` using computed keys.
 */
...
const SELECTED_CLASS = "multi-selected" as const;
export const rowClassForId = (id: GridRowId, selected: GridSelectionModel) =>
  selected.includes(id) ? SELECTED_CLASS : "";
    ...
    <div className="w-full max-w-5xl mx-auto p-4 space-y-3" data-testid="root">
      ...
      <div style={{ height: 520, width: "100%" }} className="rounded-2xl overflow-hidden shadow">
        <DataGrid
          ...
          density="compact"
          getRowClassName={(params) => rowClassForId(params.id as GridRowId, selection)}
          sx={{
            // Use computed keys so this parses correctly in TS/JSX.
            ["& ." + SELECTED_CLASS]: {
              bgcolor: "action.selected",
            },
            ["& ." + SELECTED_CLASS + ".Mui-hovered"]: {
              bgcolor: "action.selected",
            },
      ...
      <div className="text-sm opacity-70">
        ...
        <ul className="list-disc ml-5 space-y-1">
          ...
          <li>
            We add a custom row class for selected rows so ALL selected rows are visually
            highlighted (not just the last clicked).
          </li>
        </ul>
      </div>
      ...
      {process.env.NODE_ENV !== "production" && (
        <TestArea orderedIds={orderedIds} selection={selection} />
      )}
    </div>
  );
}
 ...
 */
function TestArea({ orderedIds, selection }: { orderedIds: GridRowId[]; selection: GridSelectionModel }) {
  React.useEffect(() => {
    try {
      // toggleId tests
      ...
      console.assert(JSON.stringify(toArray(5)) === JSON.stringify([5]), "toArray single value failed");

      // class assignment
      console.assert(rowClassForId(3, [1, 3, 5]) === "multi-selected", "rowClassForId for selected failed");
      console.assert(rowClassForId(4, [1, 3, 5]) === "", "rowClassForId for non-selected failed");
      console.assert(rowClassForId("a", ["a", "b"]) === "multi-selected", "rowClassForId string id failed");

      // merge uniqueness sanity
      const merged = Array.from(new Set<GridRowId>([...([1, 2, 3] as GridRowId[]), ...([2, 3, 4] as GridRowId[])]));
      console.assert(JSON.stringify(merged) === JSON.stringify([1, 2, 3, 4]), "merge uniqueness failed");
