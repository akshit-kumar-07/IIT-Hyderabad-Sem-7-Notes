...
// ---------- Static form/prompt data (unchanged) ----------
const FORMS = [
...
// ---------- PDF viewer with selective pre-render + lazy load ----------
function PdfLazyViewer({ url, preRenderPages, highlight, onRenderedPageSize }) {
  const [pdfMeta, setPdfMeta] = useState({ numPages: 0 });
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (!mounted) return;
      setPdfMeta({ numPages: doc.numPages });
      // create IO
      ...
      observerRef.current = new IntersectionObserver((entries) => {
        entries.forEach(async (entry) => {
          ...
          if (entry.isIntersecting) {
            await renderPage(doc, pageNo);
          }
        });
      }, { root: containerRef.current, threshold: 0.1 });
      // pre-render extracted pages first
      const firstBatch = [...preRenderPages].sort((a, b) => a - b);
      for (const p of firstBatch) await renderPage(doc, p);
      // observe the rest lazily
      for (let i = 1; i <= doc.numPages; i++) {
        if (!preRenderPages.has(i) && pageRefs.current[i]) {
          observerRef.current.observe(pageRefs.current[i]);
        }
      }
    }
    init();
  ...
  async function renderPage(doc, pageNumber) {
    ...
    onRenderedPageSize?.(pageNumber, { width: viewport.width, height: viewport.height });
  }
  ...
  useEffect(() => {
    ...
    if (!wrapper) return;
    // ensure page is in view if not rendered yet
    wrapper.scrollIntoView({ block: "center", behavior: "smooth" });
    // overlay
    let overlay = wrapper.querySelector(".__hl__");
    if (!overlay) {
    ...
    // position based on normalized bbox against canvas size
    const canvas = wrapper.querySelector("canvas");
    if (canvas) {
      const [x, y, w, h] = bbox;
      overlay.style.left = `${x * canvas.width}px`;
      overlay.style.top = `${y * canvas.height}px`;
      overlay.style.width = `${w * canvas.width}px`;
      overlay.style.height = `${h * canvas.height}px`;
    }
  }, [highlight]);
      ...
      <div className="h-full grid place-items-center p-8 text-center">
        <div>
          ...
          <p className="text-sm text-neutral-500 max-w-sm">
            Upload a PDF to preview it here. Extraction works best when you provide the exact document type you selected above.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 overflow-auto">
      <div className="px-3 py-4">
        {Array.from({ length: pdfMeta.numPages || 0 }).map((_, idx) => {
          const pageNo = idx + 1;
          return (
            <div
              key={pageNo}
              ...
              data-page={pageNo}
              className="relative mb-6 border border-neutral-200 rounded-xl overflow-hidden bg-white shadow-sm"
              style={{ minHeight: 240 }}
            />
          );
        })}
...
const Row = React.memo(function Row({ index, style, data }) {
  ...
  return (
    <div style={style} className="grid grid-cols-[1fr,2fr,64px] items-start gap-3 px-2 border-b last:border-b-0">
      <button
        className="text-left hover:underline py-2"
        onClick={() => f.coord && onFocus(f.coord)}
      ...
      <div className="py-2">
        <input
          ...
          onFocus={() => f.coord && onFocus(f.coord)}
...
export default function BankDocsPlayground() {
  ...
  const prePages = useMemo(() => {
    ...
    // always include page 1 as a fallback if empty
    if (s.size === 0) s.add(1);
    return s;
  }, [result]);

  // track page sizes (useful if you later want exact overlay scaling or jumps)
  ...
  const handleBack = useCallback(() => {
    setFocused(false);
    // keep results; user can re-run or adjust
  }, []);
  ...
  const PaneShell = ({ title, side, isOpen, onToggle, children }) => (
    <div className="relative overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm">
      ...
      {!isOpen && <div className="p-4 text-sm text-neutral-500">Collapsed — click the button to expand.</div>}
    </div>
  );
        ...
        <div className="mx-auto max-w-7xl px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-sm text-neutral-500 -mt-0.5">
              A safe, read-only space for users without admin privileges
            </p>
          </div>
        </div>
      </header>
          ...
          >
            ...
            {mixSelected && (
              <div className="mt-4 rounded-2xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">
                A mix of library and custom prompts is selected. The system will use the default configuration for{" "}
                <span className="font-medium">{FORMS.find((f) => f.value === selectedForm)?.label}</span>.
              </div>
            )}
          </motion.section>
        )}
          ...
          >
            <div className="px-3 pt-3">
              <div className="inline-flex rounded-xl border border-neutral-200 overflow-hidden">
                <TabButton active={previewTab === "excel"} onClick={() => setPreviewTab("excel")} icon={<TableIcon size={14} />}>
                  Excel
                </TabButton>
                <TabButton active={previewTab === "json"} onClick={() => setPreviewTab("json")} icon={<CodeIcon size={14} />}>
                  JSON
                </TabButton>
                <TabButton active={previewTab === "xml"} onClick={() => setPreviewTab("xml")} icon={<ListIcon size={14} />}>
                  XML
                </TabButton>
              </div>
            </div>
            ...
            <div className="p-4">
              {!result ? (
                ...
                <div className="overflow-auto" style={{ height: focused ? "calc(80vh - 100px)" : "calc(70vh - 100px)" }}>
                  ...
                  <VList
                    height={(focused ? 80 : 70) * 8} // ~560 or ~480 px
                ...
                <pre className="text-xs bg-neutral-50 border border-neutral-200 rounded-xl p-3 overflow-auto" style={{ height: focused ? "calc(80vh - 100px)" : "calc(70vh - 100px)" }}>
                  {jsonText}
                </pre>
              ) : (
                <pre className="text-xs bg-neutral-50 border border-neutral-200 rounded-xl p-3 overflow-auto" style={{ height: focused ? "calc(80vh - 100px)" : "calc(70vh - 100px)" }}>
