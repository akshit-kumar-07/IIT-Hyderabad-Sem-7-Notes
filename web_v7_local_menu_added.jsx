...
import { MoreHorizontal, Folder, BarChart3 } from "lucide-react";
...
function MyDocsPanel({ docs, currentUser }) {
  ...
  return (
    <div id="my-docs" className="space-y-3">
...
// ---------- Quick Menu (header small dropdown) ----------
function QuickMenu() {
  const [open, setOpen] = useState(false);
  const menuRef = useRef(null);
  useOnClickOutside(menuRef, () => setOpen(false));
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        onClick={() => setOpen(v => !v)}
        className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-sm shadow-sm hover:bg-gray-50"
        aria-haspopup="menu"
        aria-expanded={open}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -6 }}
            transition={{ duration: 0.15 }}
            className="absolute right-0 z-30 mt-2 w-48 overflow-hidden rounded-xl border border-gray-200 bg-white p-1 shadow-xl"
            role="menu"
          >
            <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-50" onClick={goMyDocs}>
              <Folder className="h-4 w-4" /> My documents
            </button>
            <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-50" onClick={() => { setOpen(false); alert('Reports'); }}>
              <BarChart3 className="h-4 w-4" /> Reports
            </button>
            <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm hover:bg-gray-50" onClick={() => { setOpen(false); alert('Templates'); }}>
...
function InlineExplorer({ status, docs, onBack, searchRef }) {
...
function NavItem({ icon: Icon, label, active = false, collapsed = false }) {
  return (
    ...
    >
      ...
      {!collapsed && <span className="hidden lg:inline font-medium">{label}</span>}
    </button>
  );
}
// ---------- Root App ----------
export default function DocFlow() {
  ...
  // app menu dropdown (beside DocFlow)
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative" ref={navRef}>
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-3 py-2 text-white shadow hover:bg-gray-800"
                  aria-haspopup="menu"
                ...
                >
                  ...
                  <span className="font-semibold tracking-tight">DocFlow</span>
                  <ChevronDown className={cn("ml-1 h-4 w-4 transition-transform", navOpen ? "rotate-180" : "")} />
                </button>
                <AnimatePresence>
                  {navOpen && (
                    <motion.div
                      ...
                      transition={{ duration: 0.15 }}
                      role="menu"
                      className="absolute left-0 z-30 mt-2 w-56 overflow-hidden rounded-xl border border-gray-200 bg-white p-1 shadow-xl"
                    >
                      <NavItem icon={LayoutGrid} label="Overview" active />
                      <NavItem icon={FileText} label="Archive" />
                      <NavItem icon={Settings} label="Settings" />
                      <div className="mt-1 border-t border-gray-100 px-3 py-2 text-[10px] text-gray-400">v1.2 • Inline</div>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
